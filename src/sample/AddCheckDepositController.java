...
public class AddCheckDepositController implements Initializable {
    @FXML TextField cdCheckNum, cdAmt, cdPayer;
    @FXML DatePicker cdDate;
    @FXML ComboBox<String> cdAcct;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        cdAcct.getItems().addAll("Checking", "Savings");
    }
    ...
    *******************************************************************************************************************/
    ...
    public void setCancelAddCDButton(ActionEvent ev) {
    ...
    public void setAddCDButton(ActionEvent ev) {
        ...
        String query = "insert into check_deposit (userID, amt, oldtotal, date, checkNum, payer, acct) values (?,?,?,?,?,?,?)";

        try {
            date = getDatePickerValue();
            acct = cdAcct.getSelectionModel().getSelectedItem().toLowerCase();
            if (acct.equals("") || checkNum.equals("") || amount.equals("") || payer.equals("") || date == null) {
                ...
                try {
                    ...
                    query = "select * from check_deposit where userID = " + userID +
                            " and checkNum = ? and amt = ? and payer = ? and acct = ? and date = ?";
                    ...
                    pStmt.setInt(1, userID);
                    pStmt.setString(2, amount);
                    pStmt.setString(3, "check_deposit");
    ...
    public String getDatePickerValue() {
        return cdDate.getValue().toString();
    }
}
